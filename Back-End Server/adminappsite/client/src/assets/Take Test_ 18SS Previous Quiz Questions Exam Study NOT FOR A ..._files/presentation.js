...
function($, sessionInfo, engineState, utils, helpitemLoader, contextlinks, mouse, settings) {
  ...
  */
  ...
  function positionTab() {
    ...
    try {
      if ($('#tab-inner').height() > 0) { // dont position if the image is not loaded yet
        if (supportTabAlign == 'right') {
          try {
            ...
            if (adjustScrollbar) {
              $('#tab-inner').css('right', scrollbarRightAdjust);
            } else {
              $('#tab-inner').css('right','0px');
            }
          } catch(e) {
            $('#tab-inner').css('right','0px');
          }
        ...
        $('#tab-inner').css('margin-top', '-' + ($('#tab-inner').height() / 2) + 'px');
        $("#tab-inner").css("display", "inline-block");
      }
    } finally {
      setTimeout(function() {
        positionTab();
  ...
  function updateTabLocker(drawMinimized) {
    ...
    $('#tab-locker').css('background-image',
        'url(//' + $('#tab-inner').data('host') + '/generateIcon.jsp'
            + '?color=' + encodeURIComponent($('#tab-inner').data('fg-color'))
            + '&size='+$('#tab-inner-img').width()
            + '&char='+characterCode+')');
  }
  ...
  */
  ...
  function showhelpRec(hid, w, h, title, tip, voting, user, shouldDisplay) {
    if (tip == "") return;
    if ($('#standardcontainer').length) return;
    if (shouldDisplay && !shouldDisplay(hid)) return;
    ...
    if (settings.SUPPORTCENTER.USEFORMESSAGES) {
      ...
      $("body").append("<div class='eesy_dark' id='dark-screen'></div>");

      if ($("#dark-screen").height() < h) {
        h = $("#dark-screen").height();
      }

      $("body").append(templates.standard);
      removeDontShowIfNoUser();

      $('#standardcontainer').css('width', w + 'px');

      $("#eesy_content").html(tip);
      $("#standardcontainer").attr('data-helpitemid', hid);
    
      addVotingBar(hid, voting, '#standardcontainer', '#standardcontainer .eesy_footer');        
    
      $("#eesy_header_text").html(title);

      $('#standardcontainer').css({
        'margin-left': -$('#standardcontainer').width() / 2 + 'px',
        'margin-top': -$('#standardcontainer').height() / 2 + 'px'
      });
    
      $('#dark-screen').fadeIn('fast');
      $('#standardcontainer').fadeIn('fast');
  ...
  function showhelpEx(hid, w, h, title, tip, voting, user, shouldDisplay) {
    if ($('#standardcontainer').length) return; // can only be one standardcontainer

    $('.eesy_dark').remove();
    showhelp(hid, w, h, title, tip, voting, user, shouldDisplay);
    $('#standardcontainer').data("helpitemid", hid);
  }
  ...
  function helpAvailableNotify() {
    ...
    var tabData = function(key) { return $('#tab-inner').data(key) };
    ...
    if (var_tab_version == 1) {
      ...
      var imageUrl = "//" + host + "/support_tab_image.jsp?"
          ...
          + "&extraHeightTop=" + ($('#tab-inner').data('hideable') ? 20 : 0)
          + "&styleChecksum=" + var_eesy_style_checksum;

      $('#tab-inner-img').one("load", function() {
      ...
      $('#tab-inner').css('background-image', 'url(' + imageUrl + ')');
      $('#tab-inner-img').attr('src', imageUrl);
      $("#tab-inner").css(supportTabAlign, "0px");
    }
  }
  ...
  $(document).ready(function() {
    utils.onClickOrSelectKey('.eesy_close', function(e, localThis) {
      RemovePresentationMode($('#standardcontainer').data("helpitemid"));
    ...
    utils.onClickOrSelectKey('.eesy_hint_hide', function(e, element) {
    ...
    utils.onClickOrSelectKey('.eesy_standard_hide', function(e, element) {
      hideAndFade('#standardcontainer', element);
    ...
    $(document).on('click', '#tab-inner', function(e) {
      launchSupportTab();
    });
    
    // TODO can we replace these handlers entirely with css pseudo class selectors? (":hover" etc)
    $(document).on("mouseenter focus", "#tab-inner", function(e) {
      $('#tab-inner').addClass("___TabIsFocused");
    });
    
    $(document).on("mouseleave blur", "#tab-inner", function(e) {
      $('#tab-inner').removeClass("___TabIsFocused");
    });
    
    $(document).on('keypress', '#tab-inner', function(e) {
    ...
    $(document).on('click', '#tab-locker', function(e) {
      if ($('#tab-inner').data('hideable')) {
        ...
        var maximizedWidth = eesy_maximizedTabWidth || ($('#tab-inner-img').width() + "px");
        
        $('#tab-inner').animate({'width': "" + (supportTabMinimized ? eesy_minimizedTabWidth : maximizedWidth)});
